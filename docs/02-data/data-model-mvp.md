...
Versión mínima viable **AgroCortex**: foto + descripción libre - diagnóstico con confianza - validación humana. Contiene  la espina dorsal relacional y los atributos necesarios para persistir la información que el MVP produce.
...
```
(Agricultor tiene Sembradio tiene Cultivo) y hace (Consulta tiene Sesión tiene Mensaje)
```
...
|---|---|---|---|
| `id` | uuid | No | PK |
| `sembradioId` | fk - Sembradio | No | N - 1 |
...
|---|---|---|---|
| Agricultor | Sembradio | 1 - N | Un agricultor tiene muchas Sembradios |
| Sembradio | Cultivo | 1 - N | Una sembradio contiene varios cultivos |
...
|---|---|---|
| Contexto ambiental (tabla) | Fuera | En MVP el contexto es sembradio (DANE + altitudM) + `fechaSiembra`/`creadoEn`. La tabla aparece cuando exista una fuente real de clima |
| Validación agronómica (tabla) | Embestido en Recomendación | Basta `estadoValidacion` + `validador` + `observaciones` |
| Catálogo DANE (tabla) | Fuera | `municipioDane` + `departamentoDane` se guardan como códigos en Sembradio; el catálogo entra cuando se necesite el select dependiente |
