...
- **Set canónico (5):** cadena de trabajo `crear-especificacion`, `escribir-plan`, `ejecutar-plan`; revisión `validar-resultado`, `auditar-contexto`.
- **Specs:** toda skill tiene su spec en el repo `~/Development/IdeaProjects/presentacion-harness` (`docs/07-work/especificacion-<skill>.html`, criterio de aceptación de la skill).
